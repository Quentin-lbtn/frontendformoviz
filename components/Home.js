...
function Home() {
  ...
  useEffect(() => {
    fetch('https://frontformoviz.vercel.app/movies') //http://localhost:3000/movies
      ...
      .then(data => {
         const newFormat = data.movies.map(movie => {
          ...
          return { title: movie.title, overview, voteAverage: movie.vote_average, voteCount: movie.vote_count, poster};
        });
        setMoviesData(newFormat);
      });
  }, []);
